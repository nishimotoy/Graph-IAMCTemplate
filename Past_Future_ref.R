...
g <- ggplot(df_GDPcapita, aes(x=Year,y=GDPcapita)) + geom_point()
plot(g)
...
g <- ggplot(df_ER, aes(x=Year,y=Electricity_Rate)) + geom_point()
plot(g)
...
g <- ggplot(df_CR_ER, aes(x=Year,y=CR_Electricity_Rate)) + geom_point()
plot(g)


#GDP_Capita vs. ChangeRate_Electricity_Rate_Total の表示 -------------------------------------------------------

df_GDP_CRER <- merge(df_GDPcapita[, !(colnames(df_GDPcapita) %in% c("VARIABLE", "UNIT"))], 
                     df_CR_ER[, !(colnames(df_CR_ER) %in% c("VARIABLE", "UNIT"))], 
                     all=T)
View(df_GDP_CRER)
write_csv(df_GDP_CRER, "./../4_output/df_GDP_CRER.csv")

g <- ggplot(df_GDP_CRER, aes(x=Year,y=GDPcapita,color=REGION)) + geom_line() + geom_point(shape=21)
plot(g)
ggsave(file = "./../4_output/Year-GDPcapita.pdf")

g <- ggplot(df_GDP_CRER, aes(x=Year,y=CR_Electricity_Rate,color=REGION)) + geom_line() + geom_point(shape=21)
plot(g)
ggsave(file = "./../4_output/Year-CR_Electricity_Rate.pdf")

g <- ggplot(df_GDP_CRER, aes(x=GDPcapita,y=CR_Electricity_Rate,color=REGION)) + 
  geom_line() + geom_point(shape=21) + scale_shape_identity()
plot(g)
ggsave(file = "./../4_output/GDPcapita-CR_Electricity_Rate.pdf")

# g <- ggplot(df_GDP_CRER, aes(x=GDPcapita,y=CR_Electricity_Rate,color=REGION)) + geom_line() + geom_point(shape = ifelse(y>0, 21, 16)) + scale_shape_identity()
# g <- ggplot(df_GDP_CRER, aes(x=GDPcapita,y=CR_Electricity_Rate,color=REGION)) + 
