...
df_past1 <- gather(df_past,key="Year",value="Value",
                            -c("MODEL","SCENARIO","REGION","VARIABLE","UNIT")) %>%  mutate(H_F="Historical")
df_future1 <- gather(df_future,key="Year",value="Value",
                              -c("MODEL","SCENARIO","REGION","VARIABLE","UNIT")) %>%  mutate(H_F="Future")
df_all <- rbind(df_past1,df_future1)

#GDP_Capita の作成と表示 -------------------------------------------------------
df_GDPcapita <- filter(df_all,VARIABLE == "GDP_Capita", SCENARIO == "Reference")
...
# ER 出力
g <- ggplot(df_GDP_ER, aes(x=Year,y=Electricity_Rate,color=REGION,shape=H_F)) + 
        geom_line() + 
        geom_point()
plot(g)
